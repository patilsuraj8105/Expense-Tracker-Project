...
export function AIInsightsPage() {
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      ...
      >
        <div className="rounded-2xl flex items-center justify-center" style={{ width: 56, height: 56, background: "linear-gradient(135deg,#00d4b4,#0891b2)" }}>
          <Wallet size={28} color="#fff" />
        </div>
        <div>
          <div style={{ color: "#e8eaf0", fontSize: 18, fontWeight: 700 }}>Expense Tracker AI Engine</div>
          <div style={{ color: "#8b9ab4", fontSize: 12, marginTop: 3 }}>
            Analyzing database history · Model status: Active · Loaded from custom classifier model
          </div>
        </div>
        <div className="ml-auto flex gap-3">
          <div className="text-center rounded-xl px-4 py-2" style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div style={{ color: "#ef4444", fontSize: 20, fontWeight: 700 }}>{anomalies.length}</div>
            <div style={{ color: "#8b9ab4", fontSize: 10 }}>Anomalies</div>
          </div>
          <button
      ...
      {trainingResult && (
        <div className="rounded-2xl p-5" style={{ background: "#111827", border: "1px solid rgba(0,212,180,0.3)" }}>
          <div className="flex items-center gap-2 mb-3">
            <CheckCircle size={16} color="#00d4b4" />
            <span style={{ color: "#e8eaf0", fontSize: 13, fontWeight: 600 }}>AI Classifier Retrained Successfully</span>
          </div>
          <div className="grid gap-4 mb-4" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
            <div style={{ color: "#8b9ab4", fontSize: 11 }}>
              Budgets Scanned: <strong style={{ color: "#e8eaf0" }}>{trainingResult.user_budgets}</strong>
            </div>
            <div style={{ color: "#8b9ab4", fontSize: 11 }}>
              Expenses Scanned: <strong style={{ color: "#e8eaf0" }}>{trainingResult.user_expenses}</strong>
            </div>
            <div style={{ color: "#8b9ab4", fontSize: 11 }}>
              Samples Trained: <strong style={{ color: "#e8eaf0" }}>{trainingResult.samples_trained}</strong>
            </div>
          </div>
          {trainingResult.feature_importances && (
            <div>
              <div style={{ color: "#8b9ab4", fontSize: 11, marginBottom: 8 }}>Feature Weights (Classifier Decision Path)</div>
              <div className="flex flex-col gap-2">
                {trainingResult.feature_importances.map((weight: number, idx: number) => (
                  <div key={idx} className="flex items-center gap-3">
                    <span style={{ color: "#8b9ab4", fontSize: 10, width: 90 }} className="truncate">
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(3,1fr)" }}>
        {[
          ...
          <div key={i} className="rounded-2xl p-4" style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div style={{ color: "#8b9ab4", fontSize: 11, marginBottom: 6 }}>{p.label}</div>
            <div style={{ color: "#e8eaf0", fontSize: 20, fontWeight: 700 }}>{p.value}</div>
            <div className="flex items-center gap-1 mt-1">
              {p.up ? <TrendingUp size={11} color="#ef4444" /> : <TrendingDown size={11} color="#00d4b4" />}
              <span style={{ color: "#8b9ab4", fontSize: 10 }}>{p.delta}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-5" style={{ gridTemplateColumns: "1.2fr 1fr" }}>
        {/* Simulator Form */}
        <div className="rounded-2xl p-5 flex flex-col gap-4" style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.07)" }}>
          <div>
            <span style={{ color: "#e8eaf0", fontSize: 14, fontWeight: 600 }}>Overspend Likelihood Simulator</span>
            <p style={{ color: "#8b9ab4", fontSize: 11, marginTop: 3 }}>
          ...
          <form onSubmit={handleSimulate} className="flex flex-col gap-3">
            <div className="flex gap-3">
              <div className="flex-1">
                <label style={{ color: "#8b9ab4", fontSize: 10, display: "block", marginBottom: 4 }}>Category</label>
                <select
                  value={simCategory}
                  onChange={(e) => setSimCategory(e.target.value)}
                  style={{ width: "100%", height: 36, background: "#1a2035", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, color: "#e8eaf0", padding: "0 8px", fontSize: 12 }}
                >
                  {CATEGORIES.map((cat) => (
              ...
              <div className="flex-1">
                <label style={{ color: "#8b9ab4", fontSize: 10, display: "block", marginBottom: 4 }}>Target Month</label>
                <select
                  value={simMonth}
            ...
            <div>
              <label style={{ color: "#8b9ab4", fontSize: 10, display: "block", marginBottom: 4 }}>Amount (₹)</label>
              <input
                type="number"
                placeholder="e.g. 5000"
                value={simAmount}
                onChange={(e) => setSimAmount(e.target.value)}
                style={{ width: "100%", height: 36, background: "#1a2035", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, color: "#e8eaf0", padding: "0 12px", fontSize: 12 }}
                required
              />
            </div>
          ...
          {simResult && (
            ...
            >
              <div className="flex items-center justify-between">
                <span style={{ color: "#e8eaf0", fontSize: 12, fontWeight: 600 }}>Classification Result</span>
                <span
              ...
              <div style={{ color: "#8b9ab4", fontSize: 11 }}>
                Model confidence: <strong style={{ color: "#e8eaf0" }}>{(simResult.confidence * 100).toFixed(1)}%</strong>
              </div>
              <div style={{ color: "#8b9ab4", fontSize: 11 }}>
                Monthly Budget: <strong style={{ color: "#e8eaf0" }}>{formatCurrency(simResult.current_budget)}</strong>
              </div>
              <div style={{ color: "#8b9ab4", fontSize: 11 }}>
                Spent before hypothetical: <strong style={{ color: "#e8eaf0" }}>{formatCurrency(simResult.current_spent)}</strong>
              </div>
            </div>
          )}
        ...
        <div className="flex flex-col gap-3">
          <span style={{ color: "#e8eaf0", fontSize: 14, fontWeight: 600 }}>Live AI Insights Feed</span>
          <div className="flex flex-col gap-3 overflow-y-auto max-h-[400px]">
            {loadingAnom ? (
              <span style={{ color: "#8b9ab4", fontSize: 12, textAlign: "center", padding: "20px 0" }}>Scanning transactions...</span>
            ) : anomalies.length === 0 ? (
              <div className="rounded-2xl p-5 flex flex-col items-center gap-3 text-center" style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.07)", padding: "40px 20px" }}>
                ...
                <div>
                  <span style={{ color: "#e8eaf0", fontSize: 13, fontWeight: 600, display: "block" }}>No Anomalies Found</span>
                  <p style={{ color: "#8b9ab4", fontSize: 11, marginTop: 4, lineHeight: 1.4 }}>
              ...
              anomalies.map((anom) => (
                <div key={anom.id} className="rounded-2xl p-5 flex items-start gap-4" style={{ background: "#111827", border: "1px solid rgba(239,68,68,0.15)" }}>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between gap-2 mb-1">
                      ...
                      <span style={{ color: "#8b9ab4", fontSize: 10, flexShrink: 0 }}>
                        {new Date(anom.expense_date).toLocaleDateString("en-IN", { month: "short", day: "numeric" })}
                      </span>
                    </div>
                    <p style={{ color: "#8b9ab4", fontSize: 12, lineHeight: 1.6 }}>
                      Transaction <strong style={{ color: "#e8eaf0" }}>"{anom.title}"</strong> for <strong style={{ color: "#ef4444" }}>-{formatCurrency(anom.amount)}</strong> was flagged as an outlier. {anom.description || ""}
                    </p>
                  </div>
                </div>
              ))
            ...
            {/* Static tips/positives */}
            <div className="rounded-2xl p-5 flex items-start gap-4" style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div className="rounded-xl flex items-center justify-center flex-shrink-0" style={{ width: 42, height: 42, background: "rgba(245,158,11,0.1)" }}>
                <Lightbulb size={20} color="#f59e0b" />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <span style={{ color: "#e8eaf0", fontSize: 13, fontWeight: 600 }}>AI Subscription Alert</span>
                  <span style={{ color: "#8b9ab4", fontSize: 10 }}>Tip</span>
                </div>
                <p style={{ color: "#8b9ab4", fontSize: 12, lineHeight: 1.6 }}>
                  Consolidate recurring monthly bills or utility costs early in the billing cycle to establish a predictable baseline budget.
                </p>
              </div>
            </div>

            <div className="rounded-2xl p-5 flex items-start gap-4" style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div className="rounded-xl flex items-center justify-center flex-shrink-0" style={{ width: 42, height: 42, background: "rgba(0,212,180,0.1)" }}>
                <ShieldCheck size={20} color="#00d4b4" />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <span style={{ color: "#e8eaf0", fontSize: 13, fontWeight: 600 }}>Duplicate Charge Protection</span>
                  <span style={{ color: "#8b9ab4", fontSize: 10 }}>Active</span>
                </div>
                <p style={{ color: "#8b9ab4", fontSize: 12, lineHeight: 1.6 }}>
